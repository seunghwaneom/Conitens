...
export function ForwardDashboardScreen() {
  // ── Zustand stores ──────────────────────────────────────────────────
  ...
  // Overview summary fetch
  useEffect(() => {
    if (route.screen !== "overview" || !config.token.trim()) {
  ...
  }, [config, route.screen, liveRevision]);

  // Inbox fetch
  useEffect(() => {
    if (route.screen !== "inbox" || !config.token.trim()) {
  ...
  // Tasks fetch (with filters)
  useEffect(() => {
    if (!config.token.trim() || (route.screen !== "tasks" && route.screen !== "task-detail")) {
    ...
    return () => { cancelled = true; };
  }, [config, route.screen, liveRevision, taskFilterOwner, taskFilterStatus, taskIncludeArchived]);
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true; };
  }, [config, route.screen, route.taskId]);
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true; };
  }, [config, route.screen, route.workspaceId]);

  // Workspace linked tasks fetch
  useEffect(() => {
    if (!config.token.trim() || route.screen !== "workspace-detail" || !route.workspaceId) {
    ...
    return () => { cancelled = true; };
  }, [config, route.screen, route.workspaceId, liveRevision]);
  ...
  // Run detail + replay + stateDocs + contextLatest fetch
  useEffect(() => {
    if (!config.token.trim() || !activeLinkedRunId) {
      setSelectedRun(null);
      setReplay(null);
      setStateDocs(null);
    ...
    Promise.allSettled([
      ...
      .then(([detailResult, replayResult, stateDocsResult, contextResult]) => {
        ...
        if (replayResult.status === "fulfilled") {
          ...
          setReplayError(null);
          const roomOpts = extractRoomOptions(replayResult.value);
          setSelectedRoomId((current) => pickNextRoomId(current, roomOpts));
        } else {
          setReplay(null);
    ...
    return () => { cancelled = true; };
  }, [activeLinkedRunId, config, liveRevision]);
  ...
  const unresolvedTaskWorkspaceRef = useMemo(() => {
    if (!taskDetail) return null;
    const workspaceId = taskDetail.stats.find((item) => item.label === "Workspace")?.value;
    if (!workspaceId || workspaceId === "none" || linkedWorkspaceOption) return null;
    return workspaceId;
  ...
  const taskRoomOptions = useMemo(() => {
    if (!taskDetail || taskDetail.linkedRoomIds.length === 0) return roomOptions;
  ...
  const taskApprovalHint = useMemo(() => {
    if (!taskDetail || !taskDetail.linkedRunId || taskSensitiveFields.length === 0) return null;
  ...
  // Prune selected task IDs when visible records change
  useEffect(() => {
  ...
  // Sync workspace draft with detail
  useEffect(() => {
  ...
  async function handleTaskQuickStatus(status: string) {
    if (!config.token.trim() || route.screen !== "task-detail" || !route.taskId) return;
    const body = {
      title: taskDraft.title,
      objective: taskDraft.objective,
      status,
  ...
  const liveStream = useForwardStream({
    ...
    enabled: Boolean(activeLinkedRunId),
    ...
    <main className="forward-main">
      ...
      {isDemo ? (
        <div className="forward-demo-banner">
          ...
          <button type="button" onClick={() => setShowConnectForm(!showConnectForm)}>
      ...
      {!isDemo || showConnectForm ? (
        <section className="forward-setup">
          <form className="forward-form" onSubmit={handleConnect}>
            <label>
              <span>API root</span>
              <input
      ...
      <section className="forward-grid">
        ...
        <section className="forward-detail">
          <div className="forward-panel-header">
            ...
            <span className={`forward-state state-${isDemo ? "ready" : route.screen === "overview" ? overviewState : route.screen === "inbox" ? inboxState : route.screen === "tasks" ? tasksState : route.screen === "task-detail" ? taskDetailState : route.screen === "workspaces" ? workspacesState : route.screen === "workspace-detail" ? workspaceDetailState : "idle"}`}>
              {isDemo ? "demo" : route.screen === "overview" ? overviewState : route.screen === "inbox" ? inboxState : route.screen === "tasks" ? tasksState : route.screen === "task-detail" ? taskDetailState : route.screen === "workspaces" ? workspacesState : route.screen === "workspace-detail" ? workspaceDetailState : "idle"}
            </span>
          </div>
          {route.screen === "overview" ? (
          ...
          {!isDemo && route.screen !== "overview" && route.screen !== "inbox" && route.screen !== "tasks" && route.screen !== "task-detail" && route.screen !== "workspaces" && route.screen !== "workspace-detail" ? (
